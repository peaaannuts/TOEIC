...
- リスニング Part 3(会話問題): PART3 8会話・計24問(2026-07-18追加)。各会話 `{ t, lines:[{s,text,jtext}], qs:[3] }`。
  `s`=話者(M/W、3人会話では M2/W2)。2件は3人会話。
- リスニング Part 4(説明文問題): PART4 6トーク・計18問(2026-07-18追加)。各トーク `{ t, lines:[{s,text,jtext}], qs:[3] }`。
  1話者(全lineのsは同一)。種別は留守電/店内アナウンス/ラジオ広告/館内ガイド/交通情報/研修冒頭の6種。
...
- **Service Workerキャッシュ**: `data.js`/`app.js` を編集したら `sw.js` の `CACHE_NAME` を必ずインクリメント
  (現在 `toeic600-v31`)。プレビューで検証する際は `navigator.serviceWorker.getRegistrations()` から
