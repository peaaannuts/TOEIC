...
- リスニング Part 3(会話問題): PART3 16会話・計48問(2026-07-18に8→16会話へ拡充)。各会話 `{ t, lines:[{s,text,jtext}], qs:[3] }`。
  `s`=話者(M/W、3人会話では M2/W2)。5件が3人会話。場面は全16件で重複なし。
- リスニング Part 4(説明文問題): PART4 12トーク・計36問(2026-07-18に6→12トークへ拡充)。各トーク `{ t, lines:[{s,text,jtext}], qs:[3] }`。
  1話者(全lineのsは同一)。種別は留守電/店内アナウンス/ラジオ広告/館内ガイド/交通情報/研修冒頭/空港搭乗/天気予報/新入社員オリエン/授賞式/機内/設備メンテの12種(重複なし)。
...
- **Service Workerキャッシュ**: `data.js`/`app.js` を編集したら `sw.js` の `CACHE_NAME` を必ずインクリメント
  (現在 `toeic600-v32`)。プレビューで検証する際は `navigator.serviceWorker.getRegistrations()` から
